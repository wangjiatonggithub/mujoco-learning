"""障碍物简单版避障，只有一个球体，提升RL泛化性，目标是障碍物在合理范围内移动RL都能找到解"""
import numpy as np
import mujoco
try:
...
class PandaObstacleEnv(gym.Env):
    def __init__(
        ...
        self.model = mujoco.MjModel.from_xml_path('./model/franka_emika_panda/scene_pos_with_obstacles.xml') # 加载环境模型
        ...
        self.obs_size = 7 + 3 + (3 * num_obstacles) + 1 # 7轴关节角度+末端到目标相对距离+末端到障碍物中心相对距离+碰撞危险指数
    ...
    def _get_observation(self) -> np.ndarray: 
        ...
        rel_goal = (self.goal_position - ee_pos).astype(np.float32) # 末端到目标的相对距离（向量）

        self.obstacle_positions = self._get_obstacle_centers()
        rel_obs = (self.obstacle_positions - ee_pos).astype(np.float32) # 末端到障碍物中心的相对距离（向量）

        min_dist = float(self._min_distance_to_obstacles(ee_pos))
        collision_risk = np.array([np.exp(-5*min_dist)], dtype=np.float32) # 碰撞危险指数（负指数）
    ...
    def _calc_reward(self, joint_angles: np.ndarray, action: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        #     distance_reward = 20.0*(1.0+(1.0-(dist_to_goal /(2*self.goal_arrival_threshold))))
